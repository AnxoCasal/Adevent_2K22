...
def printPath(file_path, last_node, visited_nodes):
    ...
    opt_path = getOptPath(last_node ,visited_nodes)
...
def getOptPath(last_node, visited_nodes):
...
def getOptStart(path):
    
    minimun = path[::-1][0]
    
    for node in path:
        if node["height"] == 1 and node["weight"] > minimun["weight"]:
            minimun = node
            
    return minimun
    
def DijkstraV2(file_path):
    ...
    print(actual_node["weight"] - getOptStart(getOptPath(actual_node, closed_nodes))["weight"])
    
DijkstraV2(".\\inputs\\hill.txt")